...
int main()
{
    ...
    for (i=0; i<NLINES; ++i)
    {
        for (j=0; j<NCOLS; ++j)
        {
            already[i][j]=-1; /// this means no cell currently has a cart on it
        }
    }
    i=j=0;
    while (fin.getline(world[i],201)) /// read each line
    {
        ...
        for (j=0; j<length; ++j)
        {
            ...
            if (iscart[ch]) /// if the character is a symbol of a cart
            {
                vcart[icart].x=i;
                vcart[icart].y=j;
                already[i][j]=icart; /// cell i,j has now a cart with the index icart
    ...
    while (1)
    {
        ...
        while (icart<ncart) /// for each cart left
        {
            ...
            already[x][y]=-1; /// no cart left on cell x,y

            if (already[xx][yy]>=0) /// if there already is a cart on cell xx,yy, meaning the new place of the cart
            {
            ...
            already[xx][yy]=icart; /// there's a cart on the cell
